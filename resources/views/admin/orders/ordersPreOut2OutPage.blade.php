        ...
        <div>
            ...
            <nav class="col-span-2">
                <ol class="list-reset py-4 pl-2 rounded flex bg-grey-light text-grey">
                    ...
                    <li class="px-2">/</li>
                    <li class="text-gray-400">出貨管理</li>
                </ol>
            </nav>

        </div>
        <div>
            <form class="hidden" ref="form_orders_id" action="" method="POST">
                @csrf
            ...
            <button v-on:click="checkPrintOrNot()" v-if="filter_target!='Owner'"
                class="btn btn-sec btn-rwd">批量列印出貨單(@{{ selectOrderIdCount . length }})</button>
            {{-- <button v-on:click="checkSentOrNot()"
                class="btn btn-fourth btn-rwd">批量出貨(@{{ selectOrderIdCount . length }})</button> --}}
        </div>
        ...
        @forelse  ($orders as $order)
            ...
            <div
                ...
                <div
                    ...
                    <div class="flex justify-between lg:block mt-1.5">
                        <div class="text-gray-350  sm:pr-2 w-full">
                            <a href="{{route('orderSinglePreOut2OutPage', ['o_id'=> $order->id])}}" class="btn btn-fourth btn-rwd">出貨</a>
...
@section('JS-content')
    ...
    <script>
        var app = new Vue({
            ...
            created: function() {
                this.filter_target = this.getUrlQuery('filter') ? this.getUrlQuery('filter') : 'HOME_TCAT';
            },
            methods: {
                ...
                checkSentOrNot: function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            const dom = vm.$refs.form_orders_id;
                            dom.action = "";
                            dom.querySelector('input[type=submit]').click()
                        }
                    })
                },
                checkPrintOrNot: function() {
                    const vm = this;
                    if (vm.selectOrderIdCount == 0) {
                        Swal.fire('請先選擇訂單編號', '', 'info')
                        return;
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            const dom = vm.$refs.form_orders_id;
                            dom.target = "print_popup"
                            dom.onsubmit = window.open('about:blank', 'print_popup',
                                'width=1000,height=800');
                            dom.action = "{{ route('printOrderShippingData') }}";
                            dom.querySelector('input[type=submit]').click()
                        }
                    })
                }
