...
@section('Breadcrumb')
    <div class="flex justify-between w-full flex-col lg:flex-row ">
        <div>
            ...
            <nav class="col-span-2">
                <ol class="list-reset py-4 pl-2 rounded flex bg-grey-light text-grey">
                    ...
                    <li class="px-2">/</li>
                    <li><a href="{{ route('coupon.show', $coupon->id) }}"
                            class="no-underline text-blue-400">{{ $coupon->title }}</a></li>
                    <li class="px-2">/</li>
                    <li class="text-gray-400">編輯</li>
                </ol>
            </nav>
        ...
        <div>
            ...
            <button v-on:click="sentEditOrNot()" class="btn btn-fourth btn-rwd">儲存</button>
...
@section('content')
    <form ref="form_save_edit" action="{{ route('coupon.update', $coupon->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="p-4 shadow-md rounded-md text-left max-w-5xl mx-auto border-solid border border-gray-300">
            <label class="block ">
                ...
                <input name="title"
                    ...
                    placeholder="EX: 好優惠券" value="{{ $coupon->title }}" />
            </label>
            ...
            <label class="block mt-4">
                <template v-if="coupon_type=='percent'">
                    ...
                    <input type="number" step="0.01" name="number"
                        class="px-3 py-1 rounded border-solid border border-gray-200 form-input mt-1 block w-full"
                        placeholder="打折數請輸入小數點，EX: 8折為0.8" value="{{ $coupon->number }}" />
                </template>
                <template v-if="coupon_type=='discount'">
                    <span class="text-gray-700">折扣數</span>
                    <input type="number" step="1" min="1" name="number"
                        class="px-3 py-1 rounded border-solid border border-gray-200 form-input mt-1 block w-full"
                        placeholder="折扣數請輸入正整數" value="{{ $coupon->number }}" />
                </template>
            </label>

            <v-date class="mt-4" name="start_at" text="開始時間" init_date="{{ $coupon->start_at }}"></v-date>
            <v-date class="mt-4" name="end_at" text="結束時間" init_date="{{ $coupon->end_at }}"></v-date>
            ...
            <label class="block mt-4">
                ...
                <input name="minimum_price"
                    class="px-3 py-1 rounded border-solid border border-gray-200 form-input mt-1 block w-full"
                    placeholder="888" value="{{ $coupon->minimum_price }}" />
            </label>
            ...
            <label class="block text-left w-full mt-4">
                ...
                <textarea name="description" class="form-textarea mt-1 block w-full px-3 py-2 rounded border border-solid " rows="10"
                    placeholder="詳細說明優惠券內容">{{$coupon->description}}</textarea>
            </label>
